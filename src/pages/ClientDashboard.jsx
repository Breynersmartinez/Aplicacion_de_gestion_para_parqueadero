...
function ClientDashboard() {
  ...
  const fetchAdmins = async () => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_URL}/Administrador/Clients`, {
        method: 'GET',
  ...
  const handleDeleteAdmin = async (idCard) => {
    if (window.confirm('¿Está seguro de que desea eliminar este clientes?')) {
      try {
        ...
        if (response.ok) {
          ...
          setError('Error al eliminar el clientes');
        }
      } catch (error) {
        console.error('Error:', error);
        setError('Error de conexión al servidor');
              ...
              <div className="bg-white shadow overflow-hidden rounded-lg">
                <div className="px-4 py-5 sm:px-6">
                  <h2 className="text-lg leading-6 font-medium text-gray-900">
                    {isEditingAdmin ? 'Editar Administrador' : 'Agregar cliente'}
                  </h2>
                </div>
